...
class MySQL:
    def __init__(self, datadir, socket, plugins=[]):
    ...
    def __enter__(self):
        ...
        if self.plugins is not None:
            for plugin in self.plugins:
                mysqld_cmdline.append("--plugin-load-add=%s" % plugin)

        self.mysql = subprocess.Popen(mysqld_cmdline)
...
def main(datadir, socket, plugins, commands):
    ...
    with MySQL(datadir, socket, plugins) as mysql:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mysql-plugin", action='append', default=[], help="MySQL plugin to load")
    parser.add_argument("command", nargs='+', help="Shell commands to execute")
    args = parser.parse_args()
    main(args.datadir, args.socket, args.mysql_plugin, args.command)

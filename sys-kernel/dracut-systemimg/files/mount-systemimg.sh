...
mount -t vfat "$SYSTEMIMG_BOOT_PARTITION" /run/initramfs/boot || die "Failed to mount boot partition"
...
mount -o loop,ro -t squashfs /run/initramfs/boot/system.img /run/initramfs/ro || die "Failed to mount system.img"
